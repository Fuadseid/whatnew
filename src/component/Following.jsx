...
import { selectVideoState, checkAuth } from "../redux/slicer";
...
import { fetchFollowingVideos } from "../redux/slicer";
  ...
  useEffect(() => {
    ...
    if (!isAuthenticated) {
      dispatch(checkAuth())
        .unwrap()
        .then(() => {
          // Only dispatch after auth is confirmed
          if (auth.user?.id) {
            dispatch(fetchFollowingVideos());
          }
        })
        .catch(() => navigate("/login"));
    } else {
      // Only dispatch if we have a user ID
      if (auth.user?.id) {
        dispatch(fetchFollowingVideos());
      }
    }
  }, [dispatch, isAuthenticated, navigate, auth.user?.id]);
  ...
  if (data.length === 0)
    return (
      <div className="flex flex-col items-center justify-center h-screen text-white bg-gray-900">
        <p className="mb-4 text-xl">No videos from users you follow</p>
        <button 
